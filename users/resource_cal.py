...
def display_maintenance_activity(request, activity_id = None):
    ...
    return render_to_response('users/rc_display_activity.html', params)
...
def _get_maintenance_activity_group_by_date(date, rank):
    ...
    if rank.upper() == 'X':
        def comp_mag(mag):
            return (TimeAgent.truncateDt(mag.get_start()) == date) and (mag.rank.upper() == 'X')
    else:
        def comp_mag(mag):
            return mag.rank.upper() == rank.upper()
