...
class BlackoutForm(forms.Form):
    ...
    until        = forms.CharField(max_length = 10)
    until_time   = forms.ChoiceField(choices = [(t, t) for t in times])
    ...
    def clean(self):
        ...
        if start is None or end is None:
            raise forms.ValidationError("ERROR: must specify Start and End")
        # start has to be a start, end has to be an end 
        if end is not None and start is not None and end < start:
            raise forms.ValidationError("ERROR: End must be after Start")
        if end is not None and until is not None and until < end:
            raise forms.ValidationError("ERROR: Until must be after End")
        # if it's repeating, we must have an until date
        if self.cleaned_data['repeat'].repeat != "Once" and until is None:
            raise forms.ValidationError("ERROR: if repeating, must specify Until")
