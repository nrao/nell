...
class PSTQueryService(PSTInterface):
    ...
    Note that this is a child of PSTInterface, as is PSTMirrorDB.
    PSTMirrorDB is a much faster interface, but we're holding on to this
    as a backup.
    """
    ...
    def getUsername(self, pst_id):
        ...
        info = self.getStaticContactInfoByID(pst_id)
        return info['account-info']['account-name']
    ...
    def getStaticContactInfo(self, key, value, use_cache = True):
        ...
        if not use_cache or cache.get(cache_key) is None:
            ...
            info = self.parseUserXML(PSTQueryService.__userDB.get_data(key, value)) or "no reservations"
